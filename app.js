...
function openEventDetail(id) {
  ...
  if (ev.groups && ev.groups.length) {
    monstersEl.innerHTML = '<div class="ev-modal-monsters-title">Monstros do evento</div>' +
      ev.groups.map(function(g) {
        const typeKey = (g.type || '').toLowerCase();
        const typeColor = TYPE_COLORS[typeKey] || '#f2a93c';
        const line = g.monsters.map(function(m, i) {
          const rawKey = m.name.toLowerCase();
          const img = getMonsterImage(typeKey, rawKey, 'default');
          const dispName = formatMonsterDisplayName(rawKey);
          const arrow = i < g.monsters.length - 1 ? '<div class="ev-evo-arrow">→</div>' : '';
          return '<div class="ev-evo-stage"><img loading="lazy" decoding="async" src="' + img + '" alt="' + esc(dispName) + '"><span>' + esc(dispName) + '</span></div>' + arrow;
        }).join('');
        return '<div class="ev-group">' +
          ...
          '<div class="ev-group-line">' + line + '</div>' +
        '</div>';
      }).join('');
  } else {
