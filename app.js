...
function loadLegendaryCups() {
  ...
  for (const weekKey in legendaryCupData) {
    ...
    legendaryCupData[weekKey].editions.forEach(function(edition, editionIndex) {
      ...
      html += '<div class="edition-container" id="edition-'+edId+'" data-week="'+weekNum+'" data-edition="'+esc(edition.name)+'" style="--card-glow:'+typeGlow+';">';
      html += '<div class="edition-hero" data-toggle-prizes>';
      html += '<img loading="lazy" decoding="async" src="'+BASE_URL+'imagens/lc_banner/'+esc(edition.banner)+'" alt="'+esc(edition.name)+'" class="edition-banner">';
      html += '<div class="edition-hero-scrim"></div>';
      ...
      html += '<div class="edition-hero-info"><span class="edition-type-badge" style="color:'+typeGlow+';border-color:'+typeGlow+'55;background:'+typeGlow+'20;">'+esc(edition.type)+' Type</span><div class="edition-name">'+esc(edition.name)+'</div></div>';
      html += '</div>';
      html += '<div class="edition-body">';
      if (hasVariants) {
      ...
      html += '<button class="mi-btn mi-open-btn" data-lc="' + esc(edition.name) + '"><span class="mi-btn-icon">📊</span> View Status</button>';
      html += '</div></div>';
    });
    html += '</div></div>';
  }
