...
// ══════════ MEGA EVOLUTION ══════════
let megaMonstersData = [];
let megaMonstersReady = fetch(BASE_URL + 'data/mega_monsters.json')
  .then(function(r){ return r.json(); })
  .then(function(data){ megaMonstersData = data; })
  .catch(function(){ console.warn('mega_monsters.json não pôde ser carregado'); megaMonstersData = []; });

let currentMegaType = 'all', currentMegaSearch = '';
let megaShyneState = {};

function renderMegaFilters() {
  const container = document.getElementById('mega-filters');
  if (!container) return;
  let html = '<button class="filter-btn potential-filter-btn active" data-type="all">✦ All Types</button>';
  MONSTER_TYPES_WITH_UMBRAL.forEach(function(t) {
    html += '<button class="filter-btn potential-filter-btn" data-type="' + t + '">' +
      '<img loading="lazy" decoding="async" src="' + BASE_URL + 'imagens/type_icon/' + t + '_type_icon.png" class="pot-filter-icon" alt="">' +
      t.charAt(0).toUpperCase() + t.slice(1) +
    '</button>';
  });
  container.innerHTML = html;
  container.querySelectorAll('.potential-filter-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      container.querySelectorAll('.potential-filter-btn').forEach(function(b){ b.classList.remove('active'); });
      this.classList.add('active');
      currentMegaType = this.dataset.type;
      displayMegaGrid();
    });
  });
}

async function loadMegaTab() {
  const grid = document.getElementById('mega-grid');
  if (!grid) return;
  createSkeletons('mega-grid', 8, 'card');
  await Promise.all([megaMonstersReady, monsterImagesManifestReady]);
  displayMegaGrid();
}

function displayMegaGrid() {
  let filtered = currentMegaType === 'all' ? megaMonstersData : megaMonstersData.filter(function(m){ return m.type === currentMegaType; });
  if (currentMegaSearch) {
    const q = currentMegaSearch.toLowerCase();
    filtered = filtered.filter(function(m) {
      return (m.monster||'').toLowerCase().includes(q) || (m.item_name||'').toLowerCase().includes(q);
    });
  }
  const grid = document.getElementById('mega-grid');
  if (!grid) return;
  if (!filtered.length) { grid.innerHTML = '<div class="no-results">Nenhuma mega evolução encontrada.</div>'; return; }

  window.__currentMegaFiltered = filtered;
  grid.innerHTML = filtered.map(function(m, idx) {
    const itemSrc = BASE_URL + 'imagens/mega_evolution_items/' + m.item;
    const monsterSrc = getMonsterImage(m.type, m.monster_key, 'default');
    return '<div class="mega-card type-' + esc(m.type) + '" data-idx="' + idx + '" tabindex="0" role="button" aria-label="' + esc(m.monster) + '">' +
      '<div class="mega-card-glow"></div>' +
      '<div class="mega-card-shine"></div>' +
      '<div class="mega-card-body">' +
        '<div class="mega-card-item-wrap"><img loading="lazy" decoding="async" src="' + itemSrc + '" class="mega-card-item-img" alt="' + esc(m.item_name) + '" onerror="this.src=\'' + BASE_URL + 'imagens/placeholder.webp\'"></div>' +
        '<div class="mega-card-bolt">⚡</div>' +
        '<div class="mega-card-monster-wrap"><img loading="lazy" decoding="async" src="' + monsterSrc + '" class="mega-card-monster-img mega-evolution" alt="' + esc(m.monster) + '"></div>' +
      '</div>' +
      '<div class="mega-card-footer">' +
        '<div class="mega-card-monster-name">' + esc(m.monster) + '</div>' +
        '<div class="mega-card-item-name">' + esc(m.item_name) + '</div>' +
        '<span class="mega-card-type-badge">' + esc(m.type) + '</span>' +
      '</div>' +
    '</div>';
  }).join('');

  grid.querySelectorAll('.mega-card').forEach(function(card) {
    card.addEventListener('click', function(){ openMegaDetail(parseInt(this.dataset.idx, 10)); });
    card.addEventListener('keydown', function(e){
      if (e.key==='Enter' || e.key===' ') { e.preventDefault(); openMegaDetail(parseInt(this.dataset.idx, 10)); }
    });
...
function openMegaDetail(idx) {
  const list = window.__currentMegaFiltered || [];
  const m = list[idx];
  if (!m) return;
  megaShyneState[idx] = false;

  const modal = document.getElementById('mega-modal');
  modal.className = 'mega-modal type-' + m.type;
  modal.dataset.idx = idx;

  document.getElementById('mega-modal-item-img').src = BASE_URL + 'imagens/mega_evolution_items/' + m.item;
  document.getElementById('mega-modal-item-img').alt = m.item_name;
  document.getElementById('mega-modal-item-name').textContent = m.item_name;
  document.getElementById('mega-modal-monster-name').textContent = m.monster;
  document.getElementById('mega-modal-type').textContent = m.type;
  const monsterImg = document.getElementById('mega-modal-monster-img');
  monsterImg.src = getMonsterImage(m.type, m.monster_key, 'default');
  monsterImg.alt = m.monster;

  const hasShyne = !!(monsterImagesManifest[m.type] && monsterImagesManifest[m.type][cleanMonsterKey(m.monster_key)] && monsterImagesManifest[m.type][cleanMonsterKey(m.monster_key)]['shyne']);
  const shyneBtn = document.getElementById('mega-shyne-toggle');
  if (hasShyne) {
    shyneBtn.style.display = '';
    shyneBtn.textContent = '✦ Toggle Shyne Variant';
  } else {
    shyneBtn.style.display = 'none';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (megaClose) megaClose.addEventListener('click', closeMegaDetail);
  const megaOverlay = document.getElementById('mega-overlay');
  if (megaOverlay) megaOverlay.addEventListener('click', function(e){ if (e.target === this) closeMegaDetail(); });
  const megaSearch = document.getElementById('mega-search');
  if (megaSearch) megaSearch.addEventListener('input', function(){ currentMegaSearch = this.value.trim(); displayMegaGrid(); });
  const shyneBtn = document.getElementById('mega-shyne-toggle');
  if (shyneBtn) shyneBtn.addEventListener('click', function() {
    const modal = document.getElementById('mega-modal');
    const idx = parseInt(modal.dataset.idx, 10);
    const list = window.__currentMegaFiltered || [];
    const m = list[idx];
    if (!m) return;
    megaShyneState[idx] = !megaShyneState[idx];
    const monsterImg = document.getElementById('mega-modal-monster-img');
    swapImgSmooth(monsterImg, getMonsterImage(m.type, m.monster_key, megaShyneState[idx] ? 'shyne' : 'default'));
    this.textContent = megaShyneState[idx] ? '✦ Show Normal Version' : '✦ Toggle Shyne Variant';
  });
});
document.addEventListener('keydown', function(e){ if (e.key === 'Escape') closeMegaDetail(); });

renderMegaFilters();

// ══════════ EGGS ══════════
let eggsData = [];
let eggsDataReady = fetch(BASE_URL + 'data/egg/eggs.json')
  .then(function(r){ return r.json(); })
  .then(function(data){ eggsData = data; })
  .catch(function(){ console.warn('eggs.json não pôde ser carregado'); eggsData = []; });

let currentEggSearch = '';

async function loadEggsTab() {
  const grid = document.getElementById('egg-grid');
  if (!grid) return;
  createSkeletons('egg-grid', 8, 'event');
  await Promise.all([eggsDataReady, monsterImagesManifestReady]);
  displayEggsGrid();
}

function displayEggsGrid() {
  let filtered = eggsData;
  if (currentEggSearch) {
    const q = currentEggSearch.toLowerCase();
    filtered = eggsData.filter(function(egg) {
      if (egg.name.toLowerCase().includes(q)) return true;
      return egg.monsters.some(function(m){ return m.name.toLowerCase().includes(q); });
    });
  }
  const grid = document.getElementById('egg-grid');
  if (!grid) return;
  if (!filtered.length) { grid.innerHTML = '<div class="no-results">Nenhum ovo encontrado.</div>'; return; }

  window.__currentEggsFiltered = filtered;
  grid.innerHTML = filtered.map(function(egg, idx) {
    const bannerUrl = BASE_URL + 'imagens/eggs/' + egg.banner;
    return '<div class="egg-card" data-idx="' + idx + '" tabindex="0" role="button" aria-label="' + esc(egg.name) + '">' +
      '<div class="egg-card-glow"></div>' +
      '<div class="egg-card-art">' +
        '<img loading="lazy" decoding="async" src="' + bannerUrl + '" alt="' + esc(egg.name) + '" class="egg-card-img">' +
      '</div>' +
      '<div class="egg-card-name">' + esc(egg.name) + '</div>' +
      '<div class="egg-card-count">' + egg.monsters.length + ' monstros</div>' +
    '</div>';
  }).join('');

  grid.querySelectorAll('.egg-card').forEach(function(card) {
    card.addEventListener('click', function(){ openEggDetail(parseInt(this.dataset.idx, 10)); });
    card.addEventListener('keydown', function(e){
      if (e.key==='Enter' || e.key===' ') { e.preventDefault(); openEggDetail(parseInt(this.dataset.idx, 10)); }
    });
  });
}

function openEggDetail(idx) {
  const list = window.__currentEggsFiltered || [];
  const egg = list[idx];
  if (!egg) return;

  document.getElementById('egg-modal-banner-img').src = BASE_URL + 'imagens/eggs/' + egg.banner;
  document.getElementById('egg-modal-banner-img').alt = egg.name;
  document.getElementById('egg-modal-title').textContent = egg.name;
  document.getElementById('egg-modal-desc').textContent = egg.description || '';
  document.getElementById('egg-modal-desc').style.display = egg.description ? '' : 'none';
  document.getElementById('egg-modal-count').textContent = egg.monsters.length + ' monstros podem nascer deste ovo';

  const monstersEl = document.getElementById('egg-modal-monsters');
  monstersEl.innerHTML = egg.monsters.map(function(m) {
    const img = getMonsterImage(m.type, m.key, m.variant);
    const typeColor = TYPE_COLORS[m.type] || '#f2a93c';
    return '<div class="egg-monster-chip" style="--m-glow:' + typeColor + ';">' +
      '<img loading="lazy" decoding="async" src="' + img + '" alt="' + esc(m.name) + '">' +
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (eggClose) eggClose.addEventListener('click', closeEggDetail);
  const eggOverlay = document.getElementById('egg-overlay');
  if (eggOverlay) eggOverlay.addEventListener('click', function(e){ if (e.target === this) closeEggDetail(); });
  const eggSearch = document.getElementById('egg-search');
  if (eggSearch) eggSearch.addEventListener('input', function(){ currentEggSearch = this.value.trim(); displayEggsGrid(); });
});
document.addEventListener('keydown', function(e){ if (e.key === 'Escape') closeEggDetail(); });
...
function loadTypeChart() {
  ...
  for (const typeName in typeData) {
    ...
    typeHTML+='<div class="type-card type-'+typeName+'" tabindex="0">' +
      '<div class="type-card-glow"></div>' +
      '<div class="type-card-shine"></div>' +
      '<div class="type-card-header">' +
        '<div class="type-hex-frame"><img loading="lazy" decoding="async" src="'+BASE_URL+'imagens/type_icon/'+typeName+'_type_icon.png" class="type-card-icon" alt="'+typeName+' type icon"></div>' +
        '<div class="type-card-name">'+typeName+'</div>' +
      '</div>' +
      '<div class="type-body">' +
        '<div class="type-info-section"><div class="type-info-label strong">⚔ Strong Against</div><div class="type-badges">'+strong+'</div></div>' +
        '<div class="type-info-section"><div class="type-info-label weak">⚠ Weak Against</div><div class="type-badges">'+weak+'</div></div>' +
      '</div>' +
    '</div>';
...
// ══════════ POTENTIALS ══════════
let allPotentials=[], currentPotentialType='all', currentPotentialSearch='';

function renderPotentialFilters() {
  const container = document.getElementById('potentials-filters');
  if (!container) return;
  const types = MONSTER_TYPES_WITH_UMBRAL;
  let html = '<button class="filter-btn potential-filter-btn active" data-type="all">✦ All Types</button>';
  ...
  container.querySelectorAll('.potential-filter-btn').forEach(function(btn){
    btn.addEventListener('click',function(){
      container.querySelectorAll('.potential-filter-btn').forEach(function(b){b.classList.remove('active');});
      this.classList.add('active');
      currentPotentialType=this.dataset.type;
      const pageEl = document.querySelector('#potentials-tab .pot-page');
      if (pageEl) pageEl.setAttribute('data-ambient', currentPotentialType);
      displayPotentials();
    });
  });
}
...
function displayPotentials() {
  let filtered=currentPotentialType==='all'?allPotentials:allPotentials.filter(function(p){return p.type===currentPotentialType;});
  if (currentPotentialSearch) {
    const q = currentPotentialSearch.toLowerCase();
    filtered = filtered.filter(function(p){
      return (p.name||'').toLowerCase().includes(q) || (p.description||'').toLowerCase().includes(q);
    });
  }
  const grid = document.getElementById('potentials-grid');
  if (!filtered.length) { grid.innerHTML = '<div class="no-results">Nenhum potencial encontrado.</div>'; return; }
  grid.innerHTML = filtered.map(function(p, idx){
    const t = p.type || 'common';
    const imgSrc = BASE_URL+'imagens/'+esc(p.image);
    return '<div class="potential-orb-card sk-'+esc(t)+'" data-idx="'+idx+'" tabindex="0" role="button" aria-label="'+esc(p.name)+'">' +
      '<div class="potential-orb-glow"></div>' +
      '<div class="potential-orb-shine"></div>' +
      '<div class="potential-orb-wrap">' +
        '<img loading="lazy" decoding="async" src="'+imgSrc+'" class="potential-orb-icon" alt="'+esc(p.name)+'" onerror="this.src=\''+BASE_URL+'imagens/placeholder.webp\'">' +
      '</div>' +
      '<div class="potential-orb-name">'+esc(p.name)+'</div>' +
      '<span class="potential-orb-type">'+esc(p.Type||p.type)+'</span>' +
      '<div class="potential-orb-desc">'+esc(p.description)+'</div>' +
    '</div>';
  }).join('');

  grid.dataset.currentList = JSON.stringify(filtered.map(function(p){ return p.name; }));
  window.__currentPotentialsFiltered = filtered;

  grid.querySelectorAll('.potential-orb-card').forEach(function(card){
    card.addEventListener('click', function(){ openPotentialDetail(parseInt(this.dataset.idx, 10)); });
    card.addEventListener('keydown', function(e){
      if (e.key==='Enter' || e.key===' ') { e.preventDefault(); openPotentialDetail(parseInt(this.dataset.idx, 10)); }
    });
    card.addEventListener('pointermove', function(e){
      if (e.pointerType && e.pointerType !== 'mouse') return;
      const rect = card.getBoundingClientRect();
...
function openPotentialDetail(idx) {
  const list = window.__currentPotentialsFiltered || [];
  const p = list[idx];
  if (!p) return;
  const t = p.type || 'common';
  const modal = document.getElementById('pot-modal');
  modal.className = 'pot-modal sk-' + t;
  document.getElementById('pot-modal-icon').src = BASE_URL+'imagens/'+esc(p.image);
  document.getElementById('pot-modal-icon').alt = p.name;
  document.getElementById('pot-modal-name').textContent = p.name;
  document.getElementById('pot-modal-type').textContent = p.Type||p.type;
  document.getElementById('pot-modal-desc').textContent = p.description || '';
  document.getElementById('pot-overlay').classList.add('open');
  document.body.style.overflow = 'hidden';
}

function closePotentialDetail() {
...
document.addEventListener('DOMContentLoaded', function() {
  const potClose = document.getElementById('pot-close');
  if (potClose) potClose.addEventListener('click', closePotentialDetail);
  const potOverlay = document.getElementById('pot-overlay');
  if (potOverlay) potOverlay.addEventListener('click', function(e){ if (e.target === this) closePotentialDetail(); });
  const potSearch = document.getElementById('potentials-search');
  if (potSearch) potSearch.addEventListener('input', function(){ currentPotentialSearch = this.value.trim(); displayPotentials(); });
});
document.addEventListener('keydown', function(e){ if (e.key === 'Escape') closePotentialDetail(); });
...
const tabState = { legendary: false, mega: false, eggs: false, events: false, skills: false, types: false, items: false, potentials: false };
...
function goToTab(target) {
  ...
  if (target === 'mega' && !tabState.mega)           { loadMegaTab(); tabState.mega = true; }
  if (target === 'eggs' && !tabState.eggs)           { loadEggsTab(); tabState.eggs = true; }
