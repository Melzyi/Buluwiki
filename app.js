...
// ══════════ MEGA ITEMS (nome + imagem do item de mega evolução) ══════════
// Antes ficava fixo em dois objetos separados (megaItemNames e megaItems);
// agora vem de um único JSON externo (data/mega_items.json).
let megaItems = {};
...
// Tenta o nome exato primeiro; se não achar, cai pro nome "limpo"
// (cobre casos como "mordrak_single" vs "mordrak_solo_04").
...
(function() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const w = (5 + Math.random() * 5).toFixed(1) + 'px';
    const h = (16 + Math.random() * 16).toFixed(1) + 'px';
    const tilt = (Math.random() * 16 - 8).toFixed(1) + 'deg';
    el.style.cssText = `left:${x}%;--dur:${dur};--pulse:${delay};--drift:${drift};--tilt:${tilt};width:${w};height:${h};`;
...
// A lista de monstros de cada aniversário agora vem de um JSON externo
// (data/anniversary_monsters.json), em vez de ficar fixa aqui no código.
let monsterData = {};
...
// A ordem das semanas/edições agora vem de um JSON externo (data/legendary_cup.json),
// pra ficar fácil reordenar sem mexer no código. Enquanto o fetch não termina,
// legendaryCupData fica vazio — renderBanner()/loadLegendaryCups() são chamados
// de novo assim que os dados chegam (ver window.__renderLCBanner / __updateLCBadge abaixo).
...
// ══════════ MONSTER IMAGES (pasta única imagens/all_monster/) ══════════
// Fonte única de verdade pra imagem de qualquer monstro do jogo, usada por
// Anniversary, Legendary Cup e pelo modal de informação. Qualquer aba nova
// que precisar de uma imagem de monstro deve usar getMonsterImage() abaixo,
// em vez de montar o caminho da pasta na mão.
...
// Alguns nomes de variante usados no código (monsterData) são diferentes
// dos nomes de pasta/arquivo usados no manifesto — mapeamos aqui.
const MONSTER_VARIANT_ALIASES = { natal: 'christmas', first_birthday: '1th' };

// Alguns monstros no monsterData/legendaryCupData estão com a grafia levemente
// diferente do nome real do arquivo no manifesto (erro de digitação antigo).
// Em vez de mudar o nome em todo lugar (o que quebraria a busca de mega item
// e outros lugares que usam esse mesmo nome), corrigimos só na hora de montar
// a imagem.
...
function getMonsterImage(type, rawName, variant) {
  ...
  // remove sufixo "_solo" (em qualquer posição) e o número final (_01, _02...)
  ...
  for (const type in data) {
    ...
    data[type].forEach(function(group, idx) {
      ...
      group.line.forEach(function(monster, i) {
        ...
        html += '<div class="evolution-stage"><img loading="lazy" decoding="async" src="'+getMonsterImage(type, monster, 'default')+'" class="evo-stage-img'+(isMega?' mega-evolution':'')+'" data-base="'+esc(monster)+'" data-type="'+esc(type)+'" data-num="'+numStr+'" alt="'+esc(formatMonsterName(monster))+'"><div class="evolution-stage-name">'+esc(formatMonsterName(monster.replace(/_solo/g,'')))+'</div></div>';
...
document.querySelectorAll('.anniversary-card').forEach(function(card) {
  ...
  card.addEventListener('click', async function() {
    await monsterDataReady; // espera o JSON de monstros de aniversário chegar
...
function getPrizeIcon(itemName) {
  ...
  // Monstros LC - Mapear para sprites (agora via getMonsterImage / pasta all_monster)
  const monsterMap = {
...
async function loadLegendaryCupWithPrizes() {
  await legendaryCupDataReady; // espera a ordem das semanas chegar do JSON
  loadLegendaryCups();
  ...
  for (const weekKey in legendaryCupData) {
    ...
    legendaryCupData[weekKey].editions.forEach(function(edition, editionIndex) {
      ...
      edition.monsters.forEach(function(monster,i) {
        ...
        html += '<div class="evolution-stage"><img loading="lazy" decoding="async" src="'+getMonsterImage(edition.type, monster.name, 'default')+'" alt="'+esc(formatMonsterName(monster.name))+'" class="legendary-monster-img'+(isMega?' mega-evolution':'')+'" data-base="'+esc(monster.name)+'" data-type="'+esc(edition.type)+'"><div class="evolution-stage-name">'+esc(formatMonsterName(monster.name))+'</div></div>';
...
let displayedSkills = [];
let skillUpgradeState = {};

// Normalizes both data/skill (capitalized keys) and data/upgrade_skill (lowercase keys) into one shape
...
function displaySkills() {
  ...
  document.getElementById('skills-grid').innerHTML = filtered.map(function(skill, idx) {
    return renderSkillCard(skill, idx, false);
  }).join('');
}

function renderSkillCard(skill, idx, showUpgrade) {
  const data = showUpgrade ? skill.upgrade : skill;
  const iconPath = showUpgrade
  ...
  const upgradeBtn = skill.upgrade
    ? '<button class="skill-upgrade-btn" onclick="toggleSkillUpgrade('+idx+')">'+(showUpgrade ? '↩ Voltar para Normal' : '⇪ Ver Upgrade')+'</button>'
    : '';
  return '<div class="skill-card sk-'+esc(skill.type)+(showUpgrade?' skill-card-upgraded':'')+'" id="skill-card-'+idx+'"><div class="skill-spine"></div><div class="skill-header"><img loading="lazy" decoding="async" src="'+iconPath+'" class="skill-book-icon" alt="'+esc(skill.type)+'"><div class="skill-title"><div class="skill-name">'+esc(data.name)+'</div><span class="skill-type-badge">'+esc(skill.type)+' type'+(showUpgrade?' · Upgrade':'')+'</span></div></div><div class="skill-stats"><div class="stat-item"><div class="stat-label">PP</div><div class="stat-value">'+esc(data.pp)+'</div></div><div class="stat-item"><div class="stat-label">Power</div><div class="stat-value power">'+esc(data.power)+'</div></div><div class="stat-item"><div class="stat-label">Precision</div><div class="stat-value precision">'+esc(data.precision)+'</div></div></div><div class="skill-effect"><strong>Effect:</strong> '+esc(data.effect)+'</div>'+upgradeBtn+'</div>';
}

function toggleSkillUpgrade(idx) {
  const skill = displayedSkills[idx];
  if (!skill || !skill.upgrade) return;
  skillUpgradeState[idx] = !skillUpgradeState[idx];
  const card = document.getElementById('skill-card-'+idx);
  if (card) card.outerHTML = renderSkillCard(skill, idx, skillUpgradeState[idx]);
}
...
function updateTabBadge(status) {
  ...
  if (status.live) {
    ...
    // Expostas globalmente pra serem chamadas de novo quando o
    // data/legendary_cup.json terminar de carregar (ver legendaryCupDataReady acima)
