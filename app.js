...
let megaItemsReady = fetch(BASE_URL + 'data/mega_items.json')
  ...
  .then(function(data){ megaItems = data; })
  .catch(function(){ console.warn('mega_items.json não pôde ser carregado'); });

// Alguns monstros "solo" tinham a chave gravada de formas diferentes no JSON
// (com ou sem "_solo"); tenta o nome exato primeiro e cai pro nome sem "_solo".
function getMegaItem(rawName) {
  return megaItems[rawName] || megaItems[rawName.replace('_solo','')] || null;
}
...
function getMonsterImage(type, rawName, variant) {
  ...
  const key = rawName.replace(/_solo/g,'').replace(/_\d+$/,'');
